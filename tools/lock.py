...
key = PBKDF2HMAC(hashes.SHA256(), 32, salt, 60000).derive(passcode.strip().lower().encode())
ct = AESGCM(key).encrypt(iv, open(src, "rb").read(), None)
digest = hashlib.sha256(ct).hexdigest()[:10]
outdir = os.path.dirname(os.path.abspath(dst))
for old in os.listdir(outdir):
    if old.startswith("sky-") and old.endswith(".bin"): os.remove(os.path.join(outdir, old))
binname = f"sky-{digest}.bin"
open(os.path.join(outdir, binname), "wb").write(ct)
...
(async () => {
  ...
  const SALT = "%SALT%", IV = "%IV%", BIN = "%BIN%";
  const b = s => Uint8Array.from(atob(s), ch => ch.charCodeAt(0));
  // Start downloading the encrypted sky right away, with progress, so it's ready by the time the passcode is typed.
  const m0 = document.getElementById("m");
  const skyPromise = (async () => {
    const res = await fetch(BIN); const total = +res.headers.get("content-length") || 0; const reader = res.body.getReader(); const chunks = []; let got = 0;
    for (;;) { const { done, value } = await reader.read(); if (done) break; chunks.push(value); got += value.length; if (total && !m0.dataset.busy) m0.textContent = "Loading the sky… " + Math.round(got / total * 100) + "%"; }
    const out = new Uint8Array(got); let o = 0; for (const c of chunks) { out.set(c, o); o += c.length; }
    if (!m0.dataset.busy) m0.textContent = ""; return out;
  })();
  skyPromise.catch(() => { m0.textContent = "Couldn't load the sky — check your connection and reload."; });
  async function open(pass) {
    ...
    const key = await crypto.subtle.deriveKey({name: "PBKDF2", salt: b(SALT), iterations: 60000, hash: "SHA-256"}, km, {name: "AES-GCM", length: 256}, false, ["decrypt"]);
    const plain = await crypto.subtle.decrypt({name: "AES-GCM", iv: b(IV)}, key, await skyPromise);
  ...
  f.addEventListener("submit", async e => { e.preventDefault(); bt.disabled = true; m.dataset.busy = "1"; m.textContent = "Opening the sky…"; try { await open(p.value); } catch (err) { m.textContent = (err && err.name === "OperationError") ? "That's not it — check the email and try again." : "Couldn't open the sky — check your connection and try again."; bt.disabled = false; delete m.dataset.busy; p.select(); } });
...
open(dst, "w").write(gate.replace("%SALT%", b64(salt)).replace("%IV%", b64(iv)).replace("%BIN%", binname))
print("gate:", os.path.getsize(dst)//1024, "KB; payload:", binname, len(ct)//1024, "KB")
